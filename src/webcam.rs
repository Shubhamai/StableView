use camera_capture::{self, Builder, ImageIterator};
use nokhwa::{Camera, CameraFormat, CameraInfo, FrameFormat, NokhwaError};
use onnxruntime::ndarray::{Array, ArrayBase, Dim, IxDynImpl, OwnedRepr};
...
pub fn initialize_nokhwa_camera(
    index: u32,
    height: u32,
    width: u32,
    fps: u32,
...
pub fn initialize_camera_capture(
    index: u32,
    height: u32,
    width: u32,
    fps: f64,
) -> Result<ImageIterator, Box<(dyn std::error::Error + 'static)>> {
    let cam_builder = camera_capture::create(index).unwrap();
